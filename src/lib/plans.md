# Planlanan Adımlar (Planned Steps)

Bu dosya, Writa uygulaması için planlanan özellikleri ve değişiklikleri içerir. Yapay zeka, bir sonraki adımları anlamak için bu dosyayı okuyacaktır. 

## Tamamlanan görevler buradan silinip `app-rules.md` dosyasına versiyon notu olarak eklenecektir.

_This file contains the planned features and changes for the Writa application. The AI will read this file to understand the next steps. Completed tasks will be removed from here and added as a version note to `app-rules.md`._

---
Yazı Tipi Ayarları kısmını sağ tarafraki tasarımlar bölümüne taşı
Metin Kutusu Ayarlarıkısmını sağ tarafraki tasarımlar bölümüne taşı
Arka Plan kısmını sağ tarafraki tasarımlar bölümüne taşı
